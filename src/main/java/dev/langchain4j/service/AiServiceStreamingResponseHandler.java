 ...
 * <p>
 * 注意：此类是从 LangChain4j main 分支提取并修改的，用于在 1.9.1 版本中支持 onPartialToolCall 功能。
...
class AiServiceStreamingResponseHandler implements StreamingChatResponseHandler {
    ...
    // 修改构造函数：添加 partialToolCallHandler 和 partialToolCallWithContextHandler 参数
    AiServiceStreamingResponseHandler(
            ...
            Consumer<PartialToolCall> partialToolCallHandler,  // 新增
            BiConsumer<PartialToolCall, PartialToolCallContext> partialToolCallWithContextHandler,  // 新增
    ...
    // 新增：onPartialToolCall 回调实现
    @Override
    public void onPartialToolCall(PartialToolCall partialToolCall) {
        LOG.debug("[AiServiceStreamingResponseHandler] onPartialToolCall 被调用: id={}, name={}, argsLen={}", 
        ...
        if (partialToolCallHandler != null) {
            ...
            LOG.warn("[AiServiceStreamingResponseHandler] 没有设置 partialToolCallHandler!");
        }
    }

    // 新增：onPartialToolCall 带 context 的回调实现
    @Override
    public void onPartialToolCall(PartialToolCall partialToolCall, PartialToolCallContext context) {
        LOG.debug("[AiServiceStreamingResponseHandler] onPartialToolCall(with context) 被调用: id={}, name={}", 
                partialToolCall.id(), partialToolCall.name());
        if (partialToolCallHandler != null) {
            partialToolCallHandler.accept(partialToolCall);
        } else if (partialToolCallWithContextHandler != null) {
    ...
    public void onCompleteResponse(ChatResponse chatResponse) {
        ...
        if (aiMessage.hasToolExecutionRequests()) {
            ...
            // 修改：递归调用时传递 partialToolCallHandler
            var handler = new AiServiceStreamingResponseHandler(
                    ...
                    partialToolCallHandler,  // 新增
                    partialToolCallWithContextHandler,  // 新增
