...
public class AppController {
    ...
    public Flux<ServerSentEvent<String>> chatToGenCode(@RequestParam Long appId,
        ...
        // 转换为ServerSentEvent类型
        return contentFlux
                .map(chunk ->{
                    // 包装成JSON对象，用d去接收流式输出信息，可以解决空值被前端丢弃的问题
                ...
                .concatWith(Mono.just(
                        // 发送结束事件done ——（区分 后端正常服务结束 or 其他异常情况（网络波动））
    ...
    public BaseResponse<Page<AppVO>> listMyAppVOByPage(@RequestBody AppQueryRequest appQueryRequest, HttpServletRequest request) {
        ...
        // 限制每页最多 20 个
        ThrowUtils.throwIf(pageSize > 20, ErrorCode.PARAMS_ERROR, "每页最多 20 条记录");
        // 分页查询
    ...
    @Cacheable(
            value = "good_app_page",
            key = "T(com.winter.labforgeai.utils.CacheKeyUtils).generateKey(#appQueryRequest)",
